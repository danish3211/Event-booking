...
import { CalendarDays, ExternalLink, Facebook, Instagram, Link as LinkIcon, MapPin, Plus, Share, Twitter } from 'lucide-react';
...
function ArtistProfile() {
    ...
    return (
        <div className="min-h-screen text-background pb-24 font-sans">
            {/* 1. Banner Section (Dark Mode theme) */}
            <div className="relative pt-32 pb-16 px-6 lg:px-20 overflow-hidden text-white">
                {/* Subtle background image overlay */}
                <div
                    className="absolute inset-0 opacity-100 object-cover w-full h-full"
            ...
            <div className="max-w-7xl mx-auto px-6 mt-16 space-y-16">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        <div className="flex items-center justify-between">
                            <h2 className="text-2xl font-black tracking-tight">Nearest show for you</h2>
                        ...
                        <div className="p-6 rounded-3xl bg-background/20 border border-surface/30 shadow-sm flex flex-col sm:flex-row items-center gap-6 hover:shadow-md transition-shadow group">
                            {/* Date Box */}
                            <div className="flex flex-col items-center justify-center shrink-0 w-24 h-24 rounded-2xl bg-surface/10 border border-surface/20">
                                <span className="text-3xl font-black">{artist.nearestShow.date.day}</span>
                                <span className="text-xs font-bold uppercase tracking-widest">{artist.nearestShow.date.month}</span>
                            </div>

                            <div className="flex-1 space-y-3 text-center sm:text-left">
                                <h3 className="text-lg font-bold leading-snug">{artist.nearestShow.title}</h3>
                                <p className="text-sm text-background/60 font-semibold flex items-center justify-center sm:justify-start gap-1.5">
                                    <MapPin size={16} className="text-background/40" /> {artist.nearestShow.location}
                                </p>
                            </div>
                            <Button
                                variant="primary"
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <h2 className="text-2xl font-black tracking-tight">About {artist.name}</h2>

                        <div className="p-8 rounded-3xl bg-background/20 border border-surface/30 shadow-sm space-y-6 h-full">
                            ...
                            <div className="pt-6 border-t border-surface/20 space-y-4">
                                <h4 className="text-[11px] font-bold text-background /50 uppercase tracking-widest">Find {artist.name} on</h4>
                                <div className="flex gap-4">
                                    {artist.socials.map((social) => {
                                        const Icon = getSocialIcon(social.id);
                                        return (
                                            <a key={social.id} href={social.link} className="p-3 rounded-full bg-surface/10 hover:bg-primary hover:text-white border border-surface/20 transition-all text-background/60 hover:scale-105 active:scale-95">
                                                <Icon size={18} />
                                            </a>
                                        );
                                    })}
                ...
                <div className="space-y-8 pt-8 border-t border-surface/20">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-2xl font-black tracking-tight">{artist.name}'s Concerts & Tour Dates</h2>
                    </div>
