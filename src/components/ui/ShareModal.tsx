        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Share & Invite your friends" size="md">
            <div className="space-y-8">
                ...
                <div className="flex items-center gap-4 p-4 rounded-2xl border border-surface/20 bg-surface/5">
                    ...
                    />
                    <div>
                        <h4 className="font-bold text-lg text-text leading-tight">{event.title}</h4>
                        <div className="flex items-center gap-2 mt-1 text-text/60 text-sm">
                ...
                {/* Link Copy Section */}
                <div className="flex items-center gap-2 p-2 pl-4 rounded-xl border border-surface/20 bg-background shadow-sm focus-within:border-primary/50 transition-colors">
                    <input
                        type="text"
                        readOnly
                        value={event.url}
                        className="flex-1 bg-transparent text-sm text-text/70 outline-none truncate"
                    />
                    <button
                        onClick={handleCopyLink}
                        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-surface/10 hover:bg-surface/20 text-text font-bold text-sm transition-all active:scale-95"
                    >
                ...
                <div className="space-y-4">
                    <p className="text-sm font-bold text-text/60 uppercase tracking-widest px-1">Social share</p>
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-6">
                        {socialPlatforms.map((platform) => (
                            <div key={platform.name} className="flex flex-col items-center gap-2 group cursor-pointer">
                                ...
                                <span className="text-[11px] font-bold text-text/60 group-hover:text-text">{platform.name}</span>
                            </div>
                        ))}
                        <div className="flex flex-col items-center gap-2 group cursor-pointer">
                            <div className="w-12 h-12 rounded-full bg-surface/10 hover:bg-surface/20 flex items-center justify-center text-text shadow-sm transition-transform group-hover:scale-110 active:scale-90 border border-surface/20">
                                <span className="font-bold text-xl leading-none">...</span>
                            </div>
                            <span className="text-[11px] font-bold text-text/60 group-hover:text-text">More</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <p className="text-sm font-bold text-text/60 uppercase tracking-widest px-1">Share with friends</p>
                    <div className="p-8 rounded-2xl border-2 border-dashed border-surface/20 bg-surface/5 flex flex-col items-center text-center space-y-4 group transition-colors hover:border-primary/30">
                        ...
                        <div className="space-y-1">
                            <p className="font-bold text-text/80">Oops! No friends found on AllEvents</p>
                            <div className="flex items-center justify-center gap-4 text-sm font-black italic">
                                <button className="text-text/40 hover:text-text transition-colors uppercase tracking-tight underline">Try Other Email</button>
